...
- **Current release:** [1MB Locator HUD 1.40.0 — Snapshot Public Beta 3](https://github.com/mrfdev/1MB-Locator-HUD/releases/tag/v1.40.0), tested and published for community feedback.
- **Previous beta:** [1MB Locator HUD 1.22.0 — Snapshot Public Beta 2](https://github.com/mrfdev/1MB-Locator-HUD/releases/tag/v1.22.0).

The feature, configuration, installation, and build references below describe the tested 1.40.0 Snapshot Public Beta 3. Please report beta feedback and problems through the [issue tracker](https://github.com/mrfdev/1MB-Locator-HUD/issues).
...
- Remappable global visibility (`F7` by default) and coordinate-copy (`F8` by default) key bindings, plus optional Mod Menu configuration with responsive scrolling, wide two-column and narrow single-column layouts, near-white setting names, color-coded state labels, and one-second hover tooltips.
...
## Installing Snapshot Public Beta 3

1. Install Fabric Loader for Minecraft 26.2.
2. Download [Fabric API](https://modrinth.com/mod/fabric-api) and [`1MB-Locator-HUD-1.40.0.jar`](https://github.com/mrfdev/1MB-Locator-HUD/releases/download/v1.40.0/1MB-Locator-HUD-1.40.0.jar).
...
Press `F8` to copy your current coordinates using the configured **Copy format** and **Decimal precision**. The binding is remappable under **Locator HUD**. This action only updates the local clipboard and shows a confirmation; it never opens chat, runs the copied command, or sends it to the server.

With Mod Menu installed, open **Mods**, select **1MB Locator HUD**, and use its configuration button. Every setting is saved immediately. The **Setup** section can apply one of four built-in presets or save and restore exactly one preferred setup. **Reset** requires confirmation before restoring factory defaults and never deletes the separate Saved setup. **Done** returns to the previous screen.

Without Mod Menu, the HUD still runs normally with its defaults or previously saved settings, and the `F7` binding remains available. Configuration is stored in `config/locator-hud.json`; the optional Saved setup uses `config/locator-hud-saved-setup.json`. Close the client before editing either file manually.
...
| Main | OW / Nether lens | `OFF` | In the vanilla Overworld, shows approximate corresponding Nether X/Z coordinates; in the vanilla Nether, shows approximate corresponding Overworld X/Z coordinates. Uses Decimal precision and does not locate portals or guarantee safety. |
...
```text
build/libs/1MB-Locator-HUD-1.40.0.jar
build/libs/1MB-Locator-HUD-1.40.0-sources.jar
build/libs/1MB-Locator-HUD-1.40.0.jar.sha256
```
...
- `src/main/java`: environment-neutral formatting, layout, option models, panel-content plans, display policies, immutable HUD snapshots, sampling schedules, settings validation, and configuration storage.
- `src/client/java`: Fabric client initialization, HUD sampling and rendering, crosshair targeting, the client configuration facade, and Mod Menu UI.
- `src/main/resources`: Fabric metadata and the mod icon.
- `src/client/resources`: client translations.
- `src/test/java`: unit tests for formatting, display modes, exhaustive row-plan matrices, geometry boundaries, responsive screen policy, visibility rules, sampling cadence, theme classification and blending, presets and Saved setup, discrete slider behavior, configuration migration, and recovery.
